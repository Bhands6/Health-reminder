...
<table>
  ...
  <tr>
    <td align="center"><img src="docs/screenshots/07-context-menu.png" width="180"/><br><b>右键菜单</b></td>
  </tr>
</table>
...
> Windows 下注册为系统级全局热键（RegisterHotKey），悬浮窗没有焦点时同样生效。
> 若某个组合键已被其他程序占用，该组合键会自动回退为窗口级快捷键，功能不受影响。
...
> 配置、统计与日志会写入 **exe 同级的 `data/`、`logs/` 目录**（首次运行自动创建），
> 不会落在 PyInstaller 的临时解压目录，因此重启后设置仍然保留。
