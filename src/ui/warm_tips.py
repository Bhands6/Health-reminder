...
class WarmTipWindow(QWidget):
    ...
    def __init__(self, tip, bg_color, parent=None, on_click=None):
...
class WarmTipController(QWidget):
    ...
    def paintEvent(self, event):
        ...
        painter.drawText(hint_rect, Qt.AlignCenter, "按空格键或点击任意窗口关闭")
    ...
    def create_one_tip(self):
        ...
        window = WarmTipWindow(tip, bg_color, on_click=self.on_space_press)
    ...
    def close_all_windows(self):
        ...
        用于重复触发时先清场，避免窗口叠加；也用于宿主窗口销毁前的清理。
        """
        self.create_timer.stop()
        self.close_timer.stop()
        self.keep_on_top_timer.stop()
        self.close_all = True
        for window in self.tips_window:
            window.close()
        self.tips_window.clear()
        self.close()
